...
function ContactHistory({ lead }: { lead: Lead }) {
  // Combine the canonical contact event with its duplicates, sorted oldest-
  // first so the customer's journey reads top-to-bottom.
  const events = [
    {
      id: lead.id,
      source: lead.source,
...
function LeadRow({
  ...
  lead: Lead
  /* Named in the pre-written text so the customer knows who is messaging
   * them from an unknown number. */
  businessName: string
  isExpanded: boolean
  isDimmed: boolean
  onToggle: () => void
